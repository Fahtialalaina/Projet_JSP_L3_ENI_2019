...
<!-- page content -->
        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                <div class="x_title">
                <h2>Modification du Versement</h2>
                <div class="clearfix"></div>
                </div>
                <div class="x_content">
                <br />
                ...
                String tmpmontant = "";
                String tmpcb = "";
                String data = "SELECT * FROM versement INNER JOIN client ON versement.cb = client.cb WHERE idve='"+num+"'";
                res = stat.executeQuery(data);
                while(res.next()){
                        tmpmontant = res.getString("montant");
                        tmpcb = res.getString("cb");
                %>
                <input type="hidden" name="idve" value='<%= res.getString("idve") %>'/>
                <div class="form-group">
                                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="nve">N°Versement <span class="required">*</span>
                                </label>
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                <input type="text" name="nve" required="required" class="form-control col-md-7 col-xs-12" value='<%= res.getString("nve") %>' readonly="readonly">
                                </div>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                <input type="text" name="cb" required="required" class="form-control col-md-7 col-xs-12"  id="cb" onchange="changer(this.id, 'nom');" value='<%= res.getString("versement.cb") %>' readonly="readonly">
                                </div>
                                </div>
                                ...
                                <select name="nom" id="nom" onchange="changer(this.id, 'cb');" class="form-control col-md-7 col-xs-12" required="required">
                                                <option value='<%= res.getString("versement.cb") %>'><%= res.getString("nom") %></option>
                                                <%
                                                while(res2.next()){
                                                %>
...
<%
String z = request.getParameter("idve");
...
if(z!=null && a!=null && b!=null && c!=null && d!=null){
    ...
    String data1 = "UPDATE versement SET nve=?, cb=?, montant=?, date=? WHERE idve='"+z+"'";
